...
/**
 * MapReduce partitioner that partitions the Mapper output such that each SolrInputDocument gets
 * sent to the SolrCloud shard that it would have been sent to if the document were ingested via the
 * standard SolrCloud Near Real Time (NRT) API. In other words, this class implements the same
 * partitioning semantics as the standard SolrCloud NRT API. This enables to mix batch updates from
 * MapReduce ingestion with updates from standard NRT ingestion on the same SolrCloud cluster, using
 * identical unique document keys.
 */
public class SolrCloudPartitioner extends Partitioner<Text, SolrInputDocumentWritable> implements
    Configurable {
  ...
  public int getPartition(Text key, SolrInputDocumentWritable value, int numPartitions) {
    ...
    // TODO: scalability: replace linear search in HashBasedRouter.hashToSlice() with binary search
    // on sorted hash ranges
    ...
    if (rootShard < 0 || rootShard >= shards) {
      throw new IllegalStateException("Illegal shard number " + rootShard + " for slice: " + slice
          + ", docCollection: " + docCollection);
    }

    // map doc to micro shard aka leaf shard, akin to HashBasedRouter.sliceHash()
    // taking into account mtree merge algorithm
    assert numPartitions % shards == 0; // Also note that numPartitions is equal to the number of
                                        // reducers
    ...
    // LOG.info("Partitioned to p: {} for numPartitions: {}, shards: {}, key: {}, value: {}",
    // microShard, numPartitions, shards, key, value);
