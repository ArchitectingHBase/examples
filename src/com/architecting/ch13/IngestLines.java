...
public class IngestLines {
	...
	public static void processVersion1() {
		...
		JavaRDD<String> textFile = jsc
				.textFile("hdfs://localhost/user/cloudera/data.txt");
	...
	public static void processVersion2() {
		...
		PairFunction<String, String, String> linesplit = new PairFunction<String, String, String>() {
		...
		Function<String, List<String>> createCombiner = new Function<String, List<String>>() {
		...
		Function2<List<String>, String, List<String>> mergeValue = new Function2<List<String>, String, List<String>>() {
			@Override
		...
		Function2<List<String>, List<String>, List<String>> mergeCombiners = new Function2<List<String>, List<String>, List<String>>() {
			@Override
		...
		JavaPairRDD<String, List<String>> keyValues = pairs.combineByKey(
				createCombiner, mergeValue, mergeCombiners);

		JavaRDD<Put> keyValuesPuts = keyValues
				.map(new Function<Tuple2<String, List<String>>, Put>() {
					@Override
					public Put call(Tuple2<String, List<String>> v1)
							throws Exception {
						...
						while (iterator.hasNext()) {
							...
							String columnQualifier = colAndVal.substring(0,
									indexDelimiter);
							String value = colAndVal
									.substring(indexDelimiter + 1);
							put.addColumn(COLUMN_FAMILY,
		...
		hbaseContext.foreachPartition(keyValuesPuts,
				new VoidFunction<Tuple2<Iterator<Put>, Connection>>() {
					@Override
					public void call(Tuple2<Iterator<Put>, Connection> t)
							throws Exception {
						Table table = t._2().getTable(TABLE_NAME);
						BufferedMutator mutator = t._2().getBufferedMutator(
								TABLE_NAME);
						while (t._1().hasNext()) {
							Put put = t._1().next();
							mutator.mutate(put);
